...
function renderResearchNotes() {
  ...
  researchNotes.forEach(note => {
    ...
    card.setAttribute("href", note.link);
    ...
    readTime.textContent = note.readTime;
    ...
    // Prevent normal link navigation if they are placeholders (#)
    if (note.link === "#") {
      card.addEventListener("click", (e) => {
        e.preventDefault();
        // Secure logging (no user object logged, just a developer debug statement)
        console.log("Research note link clicked - Placeholder state");
      });
    }
